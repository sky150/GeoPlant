...
if st.session_state.analysis_result:
    ...
    if "error" in res:
        ...
        # --- METRICS ROW ---
        st.plotly_chart(
            create_circular_gauge(score, real_data=res), use_container_width=True
        )
        ...
        if not st.session_state.regional_scan.empty:
            st.markdown("### GLOBAL SUITABILITY MAP")
            ...
            folium.Choropleth(
                ...
                fill_color="Greens",  # CONTINUOUS GREEN SATURATION
                fill_opacity=0.8,
                line_opacity=0.2,
                legend_name="Suitability Score (0-100)",
                bins=[0, 20, 40, 60, 80, 100],  # Force legend to span 0-100
            ...
            st.plotly_chart(create_top_countries_chart(top), use_container_width=True)
